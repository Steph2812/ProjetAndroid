...
class WebAdapter(private val randomUsers: User, val context: Context) : RecyclerView.Adapter<WebAdapter.WebViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WebAdapter.WebViewHolder {
        ...
        return WebViewHolder(view, randomUsers, context)
    }
    ...
    class WebViewHolder (webView: View, private val randomUsers: User, val context: Context) : RecyclerView.ViewHolder(webView) {
        ...
        fun infoUser(position: Int){
            val name = randomUsers.results[position].name.first +" "+randomUsers.results[position].name.last.toUpperCase(Locale.FRENCH)
            val location = randomUsers.results[position].location.street.name+" "+randomUsers.results[position].location.street.number + " " + randomUsers.results[position].location.state + " "+ randomUsers.results[position].location.city+" " + randomUsers.results[position].location.postcode
